...
var signIn = (function(keyspace, username, password) {
    ...
    $.ajax({
        url: "http://47.92.151.165/user/signin",
        dataType: 'json',
        type: 'POST',
        async: false,
        data: {
...
var performance = function(sessionid,term){
    let rtn = null;
    $.ajax({
        url: `http://47.92.151.165/mxobject/search?sessionid=${sessionid}&cond=${term}`,
        type: 'GET',
        dataType: 'json',
        async: false,
        beforeSend: function(xhr) {
        ...
        success: function(data, textStatus, xhr) {
            let tmp =  _.groupBy(data.message,'host');
            rtn = {target:_.keys(tmp),datapoints:}
        },
        error: function(xhr, textStatus, errorThrown) {
...
app.all('/', function(req, res) {
  ...
  res.end();
  let perf = performance(signIn,'wecise');
  console.log(123,perf)
});
