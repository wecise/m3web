 ...
 */
Sidebar.prototype.thumbHeight = 48;
 ...
 */
Sidebar.prototype.thumbBorder = 4;

/**
 * Specifies the size of the sidebar titles.
 */
Sidebar.prototype.sidebarTitleSize = 10;
...
/**
 * Adds the given image palette from matrix to the sidebar.
 */
Sidebar.prototype.addEntitiesMatrixPalette = function(id, title, prefix, type, postfix, items, titles, tags, expand)
{
    ...
    for (var i = 0; i < items.length; i++)
    {
        (mxUtils.bind(this, function(item, title, tmpTags)
        {
            ...
            fns.push(this.createVertexTemplateEntry(`shape=image;html=1;labelBackgroundColor=#ffffff;image=${window.ASSETS_ICON}/entity/png/${name}${postfix}?type=download&issys=${window.SignedUser_IsAdmin}`,
...
/**
 * Adds the given Svg palette.
 */
Sidebar.prototype.addSvgPalette = function(id, title, prefix, postfix, items, titles, tags)
{
    var showTitles = titles != null;
    var fns = [];
    ...
    for (var i = 0; i < items.length; i++)
    {
        (mxUtils.bind(this, function(item, title, tmpTags)
        {
            ...
            fns.push(this.createVertexTemplateEntry('shape=image;html=1;labelBackgroundColor=transparent;image=/web/creative/graph/' + prefix + item + postfix,
                this.defaultImageWidth, this.defaultImageHeight, '', title, title != null, null, this.filterTags(tmpTags)));
        }))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
    }
...
Sidebar.prototype.addPalette = function(id, title, expanded, onInit)
{
    ...
    if(this.imapConfig.indexOf(title) != -1){
        document.getElementById("imap-config").appendChild(elt);
    } else {
        document.getElementById("imap-shape").appendChild(elt);
    }
    ...
    //this.container.appendChild(outer);
    if(this.imapConfig.indexOf(title) != -1){
        document.getElementById("imap-config").appendChild(outer);
    } else {
        document.getElementById("imap-shape").appendChild(outer);
    }
